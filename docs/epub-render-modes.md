...
CrossDiTo normally tries to render EPUBs with the full CrossDiTo renderer. Some
...
asking CrossDiTo to simplify the layout work for that book.
...
| ---------------- | ------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| CrossDiTo Default | Most books                                                                                       | Full CrossDiTo styling, detailed CSS, table layout, image sizing, publisher spacing, Focus Reading, Guide Dots | Nothing by default                                                                            |
| Balanced         | Books where CrossDiTo has fallen back automatically, or books you want to start in a lighter mode | Publisher spacing, image sizing, decorations, Focus Reading, Guide Dots                                       | Complex CSS lookups and table layout                                                          |
...
Most users can leave books on **CrossDiTo Default**. If a book runs out of memory
while building a section, CrossDiTo will automatically try **Balanced** and then
**Light**. Manual mode changes are mainly for choosing a lighter mode before
opening a known-problem book, or for changing the saved mode after CrossDiTo has
already fallen back.

## CrossDiTo Default

CrossDiTo Default is the normal renderer.

It tries to preserve the book's layout and publisher styling as much as
CrossDiTo supports, including:
...
Use Balanced when CrossDiTo has already fallen back to it, or when you want a
...
Use Light when CrossDiTo has already fallen back to it, or when you want a
known-problem book to start in the safest mode immediately. Light may look less
like the publisher's original layout, but it gives CrossDiTo the best chance of
...
If CrossDiTo cannot build a section in Light because memory is still too tight,
...
When Safe Mode succeeds, CrossDiTo saves those settings for that book. The reader
briefly shows **Safe Mode** over the current page. Your other books keep their
own reader settings.

## Automatic Fallback

If a book starts in CrossDiTo Default and a section runs out of memory while
building, CrossDiTo automatically retries in this order:
...
If the book starts in Balanced, CrossDiTo can fall back to Light and then Safe
Mode. If the book starts in Light, CrossDiTo can still try Safe Mode.

When a fallback succeeds, CrossDiTo saves that successful mode for the book. The
...
CrossDiTo already falls back automatically, but it is useful when you know a book
