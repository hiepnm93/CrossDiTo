...
CrossDiTo readers. It uses ESP-NOW, so it does not need a Wi-Fi network,
internet connection, account, or computer.

## Requirements

- Both readers must run compatible CrossDiTo builds and be nearby.
...
3. To change where received files are saved, select **Change folder** before
   starting. CrossDiTo remembers this folder; the default is the SD-card root.
...
CrossDiTo asks whether to **Replace** it, **Keep both**, or cancel. **Keep both**
...
CrossDiTo writes incoming data to a temporary file and verifies its size and
...
together, and both run compatible CrossDiTo builds. Exit and reopen the transfer
