...
CrossDiTo can share the current EPUB reading position between two nearby X4 Pro readers running CrossDiTo. The sync is direct reader-to-reader over ESP-NOW; it does not use WiFi, a server, an account, or KOReader's online sync service.
...
- Both readers must be running CrossDiTo.
...
Before starting the sync, CrossDiTo saves the current reader progress. The sender then broadcasts the current EPUB position over ESP-NOW. The receiver checks that the incoming packet is from a compatible CrossDiTo nearby-position protocol and that the book identity matches the EPUB currently open on the receiver.

If the book matches, the receiver maps the shared KOReader-compatible position back to CrossDiTo's local section and page model. When paragraph, list-item, or anchor details are available, CrossDiTo uses them to refine the target page.

After you press **Select**, CrossDiTo writes the received position to the normal EPUB progress file and returns to the reader at that position.
...
The other reader is using an incompatible nearby-position sync protocol. Update both readers to compatible CrossDiTo builds.
...
This feature depends on ESP-NOW hardware support, so it only works on a real X4 Pro.
