...
Before CrossDiTo can display an EPUB chapter, it lays out the chapter into
pages and saves that layout in the book's cache. **Indexing Method** chooses
whether CrossDiTo finishes that work before you start reading the chapter, or
...
| Incremental  | Builds enough pages to show your current position, then continues in small background steps while you read. | You can begin reading a large chapter sooner.                   | If you reach pages that have not been built yet, CrossDiTo may briefly show **Indexing** while it catches up. |

## Full Section

Full Section finishes the complete chapter cache before CrossDiTo displays an
...
CrossDiTo also tries to build the next chapter while you are reading the
...
best-effort: when the X4 Pro lacks enough free or contiguous memory, CrossDiTo skips that background work
...
Incremental builds only as far as CrossDiTo needs to display the current page.
...
can start. CrossDiTo saves the pages it has already built, so leaving the book
...
stay ahead. That is normal: CrossDiTo is building just enough additional pages
...
prefix, CrossDiTo indexes forward until that location is available. Switching
...
choice is used the next time CrossDiTo needs to index a chapter. It also does
...
Indexing Method changes _when_ CrossDiTo performs layout work; it does not
